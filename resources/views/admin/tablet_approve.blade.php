                ...
                <div class="table-responsive">
                  <table id="datatable" class="table table-striped table-bordered">
                    ...
                    <td>{{$key+1}}</td>
                        ...
                        <td><form onsubmit="return confirm('Are you sure want to approve this post')" action="{{route('tablet_approval')}}" method="POST">
                        @csrf
                        ...
                        <td>                        
                        <img src="{{asset('uploadtablet/tablet1/' .$data->image1) }}" width="100px;" height="100px;" name="image1">
                        </td>
                        <td>{{$data->name}}</td>
                        <td>{{$data->number}}</td>
                        <td>
                        <a href="delete-tablet/{{$data->id}}" onclick="return confirm('are you sure?')" class="btn btn-danger">Delete</a>
                      </td>
                    </tbody>
                    @endforeach
                  </table>
